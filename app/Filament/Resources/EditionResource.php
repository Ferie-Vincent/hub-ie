...
class EditionResource extends Resource
{
    ...
    {
        return $form->schema([
            Forms\Components\Tabs::make('Édition')
                ->tabs([
                    ...
                    // ── Onglet 2 : Contenu du site ────────────────────────
                    Forms\Components\Tabs\Tab::make('Contenu site')
                        ...
                        ->schema([
                            ...
                            Forms\Components\Section::make('Hero — Textes')
                                ...
                                ->schema([
                                    ...
                                    Forms\Components\Repeater::make('dates_cles')
                                        ...
                                        ->schema([
                                            Forms\Components\TextInput::make('lieu')->label('Lieu')->required()->columnSpan(1),
                                            Forms\Components\TextInput::make('sublabel')->label('Libellé')->required()->columnSpan(2),
                                            Forms\Components\TextInput::make('date')->label('Date affichée')->required()->columnSpan(1),
                                        ])
                                        ->columns(4)
                                        ->maxItems(6)
                            ...
                            Forms\Components\Section::make('Mot du Ministre')
                                ...
                                ->schema([
                                    ...
                                    Forms\Components\Repeater::make('minister_speech')
                                        ...
                                        ->schema([
                                            Forms\Components\Textarea::make('text')->label('Paragraphe')->rows(4)->required(),
                                        ])
                                        ->maxItems(6)
                            ...
                            Forms\Components\Section::make('Statistiques (section Ministre)')
                                ...
                                ->schema([
                                    Forms\Components\Repeater::make('stats_cards')
                                        ...
                                        ->schema([
                                            Forms\Components\TextInput::make('value')->label('Valeur')->required()->columnSpan(1),
                                            Forms\Components\TextInput::make('label')->label('Intitulé')->required()->columnSpan(2),
                                            Forms\Components\TextInput::make('caption')->label('Légende')->columnSpan(2),
                                            Forms\Components\Select::make('color')->label('Couleur')
                                                ->options(['orange' => 'Orange', 'vert' => 'Vert'])->default('orange')->columnSpan(1),
                                        ])
                                        ->columns(6)
                                        ->maxItems(4)
                            ...
                            Forms\Components\Section::make('Cap stratégique — Objectifs')
                                ->schema([
                                    Forms\Components\Repeater::make('cap_objectifs')
                                        ...
                                        ->columns(4)->maxItems(6)->addActionLabel('Ajouter un item')->collapsed(),
                                ]),
                            ...
                            Forms\Components\Section::make('Institutions partenaires')
                                ->description('Cartes affichées dans la section patronage/parrainage de la page d\'accueil.')
                                ->schema([
                                    Forms\Components\Repeater::make('institutions')
                                        ->label('Cartes institution')
                                        ->schema([
                                            Forms\Components\TextInput::make('kicker')->label('Kicker')->required()->columnSpan(1),
                                            Forms\Components\Select::make('type')->label('Accent')
                                                ->options(['orange' => 'Orange', 'vert' => 'Vert'])->default('orange')->columnSpan(1),
                                            Forms\Components\TextInput::make('title')->label('Titre (HTML: <br> autorisé)')->required()->columnSpan(2),
                                            Forms\Components\Textarea::make('text')->label('Texte')->rows(2)->required()->columnSpanFull(),
                                            Forms\Components\TextInput::make('footer')->label('Pied de carte')->columnSpanFull(),
                                        ])
                                        ->columns(4)
                                        ->maxItems(4)
                                        ->addActionLabel('Ajouter une institution')
                                        ->collapsed()
                                        ->columnSpanFull(),
                                ]),

                            Forms\Components\Section::make('Formats d\'échange')
                                ->description('Les 4 cartes de la section "Comment ça marche ?" (Plénière, Atelier, Panel, B2B).')
                                ->schema([
                                    Forms\Components\Repeater::make('formats_echange')
                                        ->label('Formats')
                                        ->schema([
                                            Forms\Components\TextInput::make('num')->label('N°')->default('01')->columnSpan(1),
                                            Forms\Components\TextInput::make('kicker')->label('Type (ex: Plénière)')->required()->columnSpan(1),
                                            Forms\Components\TextInput::make('title')->label('Titre')->required()->columnSpan(2),
                                            Forms\Components\Textarea::make('description')->label('Description')->rows(2)->required()->columnSpanFull(),
                                        ])
                                        ->columns(4)
                                        ->maxItems(4)
                                        ->addActionLabel('Ajouter un format')
                                        ->collapsed()
                                        ->columnSpanFull(),
                                ]),

                            Forms\Components\Section::make('Newsletter')
                                ->description('Textes de la section d\'invitation à s\'inscrire.')
                                ->schema([
                                    Forms\Components\TextInput::make('newsletter_title')
                                        ->label('Titre')
                                        ->placeholder('Rejoignez le Hub.'),

                                    Forms\Components\Textarea::make('newsletter_subtitle')
                                        ->label('Sous-titre')
                                        ->rows(2)
                                        ->placeholder('Recevez le programme officiel détaillé…')
                                        ->columnSpanFull(),
                                ])
                                ->columns(1),
                    ...
                    // ── Onglet 3 : Programme ──────────────────────────────
                    Forms\Components\Tabs\Tab::make('Programme')
                        ->icon('heroicon-o-calendar-days')
                        ->badge('4 jours')
                        ->schema([

                            Forms\Components\Section::make('Jour 1 — 22 juin · Ouverture')
                                ->schema([self::programmeDayRepeater('programme_j1')])
                                ->collapsible(),

                            Forms\Components\Section::make('Jour 2 — 23 juin · Ateliers')
                                ->schema([self::programmeDayRepeater('programme_j2')])
                                ->collapsible()
                                ->collapsed(),

                            Forms\Components\Section::make('Jour 3 — 24 juin · Ateliers')
                                ->schema([self::programmeDayRepeater('programme_j3')])
                                ->collapsible()
                                ->collapsed(),

                            Forms\Components\Section::make('Jour 4 — 25 juin · Clôture')
